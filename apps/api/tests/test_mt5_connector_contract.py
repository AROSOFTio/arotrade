...
class MT5ConnectorContractTests(unittest.TestCase):
    def test_post_body_excludes_string_terminator_in_every_distribution(self):
        ...
        self.assertIn("StringToCharArray(payload, encoded, 0, WHOLE_ARRAY", sources[0])
        self.assertIn("ArrayCopy(data, encoded, 0, 0, bodyBytes)", sources[0])
        self.assertNotIn("StringToCharArray(payload, data", sources[0])
