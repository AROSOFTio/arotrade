  ...
  return (
    <main className="flex min-h-screen bg-[#f6f7f9] px-4 py-8 sm:items-center sm:justify-center">
      <section className="mx-auto w-full max-w-md">
        <Link href="/" className="mb-8 inline-flex items-center gap-3"><img src="/logo.png" alt="AroPilot logo" className="h-10 w-10" /><span><span className="block text-base font-bold text-slate-950">AroPilot</span><span className="block text-xs font-medium text-slate-500">by AROFi</span></span></Link>
        <div className="card"><div className="flex h-11 w-11 items-center justify-center rounded-lg bg-blue-50 text-[#2563eb]"><LockKeyhole size={21} aria-hidden="true" /></div><h1 className="mt-5 text-2xl font-bold text-slate-950">Sign in</h1><p className="mt-2 text-sm leading-6 text-slate-500">Open your MT5 intelligence workspace.</p>{error && <div className="mt-5 rounded-md border border-red-200 bg-red-50 px-3 py-3 text-sm text-red-700">{error}</div>}<form onSubmit={handleSubmit} className="mt-6 space-y-4"><div><label htmlFor="email" className="label">Email address</label><input id="email" type="email" autoComplete="email" className="input-base" value={email} onChange={(event) => setEmail(event.target.value)} required /></div><div><label htmlFor="password" className="label">Password</label><input id="password" type="password" autoComplete="current-password" className="input-base" value={password} onChange={(event) => setPassword(event.target.value)} required /></div><button type="submit" disabled={loading} className="btn-primary mt-2 w-full">{loading ? 'Signing in…' : <>Sign in <ArrowRight size={16} aria-hidden="true" /></>}</button></form><p className="mt-6 text-center text-sm text-slate-500">New to AroPilot? <Link href="/register" className="font-semibold text-[#2563eb] hover:text-[#1d4ed8]">Create an account</Link></p></div>
      </section>
    </main>
  )
}
